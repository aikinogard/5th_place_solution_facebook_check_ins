# scan the ensemble to see whether we should add one to the ensemble
...
if __name__ == "__main__":
    ...
    add_list = ["../single_model/svc/svc_rbf_xyat_v",
                "../single_model/svc/svc_rbf_xyatu_v"]

    logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)s: %(message)s')
    rec = []
    for add_model in add_list:
        # ridx is the index of model added to the ensemble
        probas = None
        ...
        logging.info(" ")
        logging.info("scan result: %s, %f" % (add_model, valid_score))
        logging.info(" ")
        rec.append((add_model, valid_score))
        logging.info("CURRENT SORTED RESULT:")
